$.widget("forecast.assemblyPo", {
    options: {
        ...
        messages: undefined,
        viewOnly:undefined,
        labelName: ['Movandi Information', 'Ship to Information', 'Vendor Information'],
        addressItem: {
            id: '',
            organization: '',
            addressBookType: '',
    ...
    _create: function () {
        console.log("SMNLOG :: ---------- assemblyPo widget create ----------");
        ...
        self.el.assemblyPoForm = $("#assembly_po_form");
        ...
        self.el.addressInfoItemDiv = $('.address-item-div');
        self.el.addressInfoModalWrapper = $(".addressInfoModalWrapper");

        self.el.gobackButton = '<a href="/admin/assemblyPOList" type="button" class="btn btn-primary float-right" ><i class="fa fa-arrow-left"></i>&nbsp;' + self.options.messages.goBack + '</a>';
    ...
    _init: function () {
        console.log("SMNLOG :: ---------- assemblyPo widget init ----------");
        var options = this.options;
        var self = this;
        ...
        if (self.options.addressItemListSize > 0) {
            $.each(self.options.addressItemList, function (index, addressItem) {
                self.addAddressInformationRow(index, addressItem, self.options.labelName[index]);
            });

        } else {
            $.each(self.options.filteredOrgList, function (idx) {
                self.addAddressInformationRow(idx, self.options.addressItem, self.options.labelName[idx]);
            });
        }
    ...
    initiateFormValidation: function () {
        ...
        self.el.assemblyPoForm.validate(validateJson);
    ...
    uiEventInitialization: function () {
        ...
        self.el.submitButton.on('click', function(){
            self.el.assemblyPoForm.trigger('submit');
        });
    },
    addAddressInformationRow: function (idx, addressItem, labelName) {
        var self = this;
        var index = self.el.addressInfoItemDiv.find('div.form-group').length;
        var $rowToAppend = '<div class="form-group row organizationRow" data-attr-index="'+index+'">'
            ...
            + '</div>';
        self.el.addressInfoItemDiv.append($rowToAppend);
    },
    getSelectBoxForOrganization: function (index, rowIndex, selectedItem) {
        ...
        $.each(self.options.filteredOrgList[rowIndex], function (index, item) {
    ...
    disableForViewOnly: function(){
        ...
        self.el.assemblyPoForm.find('input[type!="hidden"]').attr("disabled", true);
        self.el.assemblyPoForm.find('select').attr("disabled", true);
        self.el.assemblyPoForm.find('textarea').attr("disabled", true);
        self.el.searchBoxReviewer.attr("disabled", true);
        self.el.assemblyPoForm.find('a[type="button"]').not("#submitButton").hide();
        self.el.assemblyPoForm.find('button[type="button"]').not(".next-button").hide();
        self.el.submitButton.parent().html(self.el.gobackButton);
        self.el.assemblyPoForm.attr("href", "#");
    },
    destroy: function () {
    }
});
