$.widget("forecast.bumpWo", {
    ...
    _create: function () {
        console.log("SMNLOG :: ---------- bumpWo widget create ----------");
        ...
        self.el.bumpWoForm = $("#bump_wo_form");
        self.el.id = $(document).find("#id");
        self.el.workOrderName = $("#bumpWorkOrderName");
        ...
        self.el.gobackButton = '<a href="/admin/bumpWOList" type="button" class="btn btn-primary float-right" ><i class="fa fa-arrow-left"></i>&nbsp;' + self.options.messages.goBack + '</a>';
    ...
    _init: function () {
        console.log("SMNLOG :: ---------- bumpWo widget init ----------");
        var options = this.options;
        var self = this;
        console.log("AFRILOG ::"+JSON.stringify(self.options.finishedGoodsList));
    ...
    initiateFormValidation: function () {
        ...
        var validateJson = {
            ...
            rules: {
                ...
                'productionPlan.id': {
                    required: true,
                    isBumpAvailableInProductionPlan: true
                },
                manufacturingSite: {
                    required: true
                },
        ...
        $.validator.addMethod("checkTapeOutCount", function (value, element) {
            ...
            $.ajax({
                type: "GET",
                url: "/bumpWo/tapeOutCount/check?poId="+poId+"&waferTypeId="+waferTypeId+"&currentWorkOrderId="+currentWorkOrderId,
        ...
        $.validator.addMethod("checkTapeOutCost", function (value, element) {
            ...
            $.ajax({
                type: "GET",
                url: "/bumpWo/tapeOutCost/check?poId="+poId+"&waferTypeId="+waferTypeId+"&currentWorkOrderId="+currentWorkOrderId,
                success: function (response) {
        ...
        $.validator.addMethod("isBumpAvailableInProductionPlan", function (value, element) {
            errorMsg = '', $valid = false;
            $.ajax({
                type: "GET",
                url: "/bumpWo/isBumpAvailable?productionPlanId="+value,
                success: function (response) {
                    $valid = response;
                    if(response === false){
                        errorMsg = "Bump Service is not available in this production plan."
                    }
                },
                async: false
            });
            $.validator.messages["isBumpAvailableInProductionPlan"] = errorMsg;
            return $valid;
        }, '');

        self.el.bumpWoForm.validate(validateJson);
    ...
    uiEventInitialization: function () {
        ...
        self.el.submitButton.on('click', function(){
            self.el.bumpWoForm.trigger('submit');
        });
    ...
    disableForViewOnly: function(){
        ...
        self.el.bumpWoForm.find('input[type!="hidden"]').attr("disabled", true);
        self.el.bumpWoForm.find('select').attr("disabled", true);
        self.el.bumpWoForm.find('textarea').attr("disabled", true);
        // self.el.searchBoxReviewer.attr("disabled", true);
        self.el.bumpWoForm.find('a[type="button"]').not("#submitButton").hide();
        self.el.submitButton.parent().html(self.el.gobackButton);
        self.el.bumpWoForm.attr("href", "#");
    },
    showErrorCountInTabHeader: function(errorMap, errorList){
        var self = this;
        self.data.tabWiseErrorCountMap = {};
        self.el.bumpWoForm.find("div.tab-content").find('.tab-pane').find(".error:not(label)").each(function(){
        ...
        $.each(self.data.tabWiseErrorCountMap, function(key, value){
            self.el.bumpWoForm.find("ul.nav-tabs").find("#"+key+"-tab").html(self.options.tabHeaderValueMap[key+'-tab']+"&nbsp;<font style='color: #dd0000;'>("+ value +")</font>")
        });
    },
    keepDefaultValueForTabHeader: function(){
        var self = this;
        $.each(self.options.tabHeaderValueMap, function(key, value){
            self.el.bumpWoForm.find("ul.nav-tabs").find("#"+key).html(self.options.tabHeaderValueMap[key]);
        });
    },
    destroy: function () {
    }
