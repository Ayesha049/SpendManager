$.widget("forecast.fabWipReconciliation", {
    options: {
        messages: undefined,
        fabWoId: 0
    },
    _create: function () {
        console.log("SMNLOG :: ---------- Fab WIP Reconciliation widget create ----------");
        var self = this;
        self.el = {};
        self.data = {};
        ...
        self.el.workOrderSelect.change(function () {
            ...
            self.makeWipReconciliationDetailsTable(fabWorkOrderId);
        });
        ...
        if(self.options.fabWoId > 0){
            console.log("SMNLOG fabWoId::"+self.options.fabWoId);
            self.el.workOrderSelect.val(self.options.fabWoId).trigger('change');
        }
    },
    _init: function () {
        console.log("SMNLOG :: ---------- Fab WIP Reconciliation init ----------");
        var self = this;
    ...
    makeWipReconciliationDetailsTable: function (fabWorkOrderId) {
        var self = this;
        var listSize = 0;
        ...
        if (fabWorkOrderId > 0) {
            ...
            $.get('/admin/getFabWipInventoryForReconciliation', {fabWorkOrderId: fabWorkOrderId}, function (list) {
    ...
    getTableRowHtmlByProductItem: function(item, index, recincileSuccess){
        ...
        html += ' data-product-item-id="'+item.id+'" data-service-execution-plan-id="'+item.serviceExecutionPlanId+'" '
            + ' data-in-progress-qty="'+item.inProgressQuantity+'" data-fabWorkorder-id="'+item.fabWorkOrderId+'" '
    ...
    singleReconcile: function(fabWorkOrderId, productItemId, serviceExecutionId, reconcileQty, rowObject){
        var self = this;
        var validator = self.el.singleReconcileForm.validate();
        Forecast.APP.startLoading();

        $.get('/admin/reconcileFabService',
            {
                fabWorkOrderId: fabWorkOrderId,
    ...
    singleReconcileBtnEvent: function(trObject, reconcileAllBtn){
        var self = this;
        var fabWorkOrderId = +trObject.attr('data-fabWorkorder-id');
        ...
        if(isNaN(parseInt(reconcileQty)) || parseInt(reconcileQty) == 0 || !Forecast.APP.isDigits(reconcileQty)){
            ...
            self.singleReconcile(fabWorkOrderId, productItemId, serviceExecutionPlanId, parseInt(reconcileQty), trObject)
        }
    },
    destroy: function () {
    }
