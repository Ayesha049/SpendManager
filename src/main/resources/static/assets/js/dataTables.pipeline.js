...
$.fn.dataTable.pipeline = function ( opts ) {
    ...
    return function ( request, drawCallback, settings ) {
        ...
        // When request.length = -1, means all data, then need to update request.length from cache, otherwise it will
        // deduct the last item from the aaData list
        if(request.length == -1){
            ajax = true;
        ...
        if ( ajax ) {
            ...
            settings.jqXHR = $.ajax( {
                ...
                "success":  function ( json ) {
                    ...
                    // deduct the last item from the aaData list
                    if(requestLength == -1){
                        requestLength = json.aaData.length;
