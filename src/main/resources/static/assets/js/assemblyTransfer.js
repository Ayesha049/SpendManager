$.widget("forecast.assemblyTransfer", {
    options: {
        messages: undefined
    },
    _create: function () {
        console.log("SMNLOG :: ---------- ASSEMBLY Transfer widget create ----------");
        var self = this;
        self.el = {};
        self.data = {};

        // UI element Initialization
        self.el.assemblyTransferForm = $("#transferForm");
        ...
        self.el.workOrderSelect.change(function () {
            var assemblyWorkOrderId = +$(this).val();
            console.log("select assemblyWorkOrderId :::: " + assemblyWorkOrderId);
            self.makeWipTransferDetailsTable(assemblyWorkOrderId);
        });
        ...
        $(document).on('click', '.statusChangeTransferProductItem', function(){
            console.log("SMNLOG :: status btn is clicked");
            console.log("SMNLOG :: new status:::: "+ $(this).attr('status'));
            self.changeStatusTransferProductItemBtnEvent($(this).closest('tr'), $(this).attr('status'));
        });
        
    },
    _init: function () {
        console.log("SMNLOG :: ---------- ASSEMBLY WIP Transfer init ----------");
        var self = this;

    },
    makeWipTransferDetailsTable: function (assemblyWorkOrderId) {
        var self = this;
        var listSize = 0;

        if (assemblyWorkOrderId > 0) {
            var html = '<table class="table table-striped custom-table">';
            html += '<thead>';
            html += '<tr>';
            html += '<th>SL#</th>';
            html += '<th>' + self.options.messages.deviceNumber + '</th>';
            ...
            $.get('/admin/getAssemblyWipInventoryForTransfer', {assemblyWorkOrderId: assemblyWorkOrderId}, function (list) {
                ...
                self.generateTransferStatusTable(assemblyWorkOrderId, listSize);
    ...
    generateTransferStatusTable: function(assemblyWorkOrderId, transferTableDetailsSize){
        var self=this;
        var html = '<table class="table table-striped custom-table">';
        html += '<thead>';
        html += '<tr colspan="4">'+self.options.messages.transferTableColspanHeader+'</tr>';
        html += '<tr>';
        html += '<th>SL#</th>';
        html += '<th>' + self.options.messages.transferTableDeviceNumber + '</th>';
        ...
        $.get('/admin/getAssemblyWipInventoryStatusForTransfer', {assemblyWorkOrderId: assemblyWorkOrderId}, function (list) {
    ...
    changeStatusTransferProductItemBtnEvent: function(trObject, status){
        ...
        var validator = self.el.assemblyTransferForm.validate();
        var errorMessage = "";
    ...
    recordStatusChange:function(trObject, productItemId, productTransferItemId, status, transferredQuantity, wasteQuantity){
        ...
        $.get('/admin/transferAssemblyServiceChangeProductTransferItemStatus',
            {
            ...
            function(response){
                ...
                self.el.assemblyTransferForm.validate();
    ...
    transferProductItem: function(trObject, productItemId, transferQty){
        ...
        $.get('/admin/transferAssemblyService',
            {
                productItemId: productItemId,
    ...
    initiateFormValidation: function (transferTableDetailsSize, transferStatusTableSize) {
        ...
        self.el.assemblyTransferForm.validate(validateJson);
    },
