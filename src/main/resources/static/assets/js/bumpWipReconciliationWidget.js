$.widget("forecast.bumpWipReconciliation", {
    options: {
        messages: undefined,
        bumpWoId: 0
    },
    _create: function () {
        console.log("SMNLOG :: ---------- Bump WIP Reconciliation widget create ----------");
        var self = this;
        self.el = {};
        self.data = {};
        ...
        self.el.createdDate = $("#createdDate");
        ...
        self.el.workOrderSelect.change(function () {
            var bumpWorkOrderId = +$(this).val();
            self.makeWipReconciliationDetailsTable(bumpWorkOrderId);
        });
        ...
        if(self.options.bumpWoId > 0){
            console.log("SMNLOG bumpWoId::"+self.options.bumpWoId);
            self.el.workOrderSelect.val(self.options.bumpWoId).trigger('change');
        }
    },
    _init: function () {
        console.log("SMNLOG :: ---------- Bump WIP Reconciliation init ----------");
        var self = this;
    ...
    uiEventInitialization: function () {
        ...
        self.el.createdDate.datetimepicker({
            format: Forecast.APP.GLOBAL_DATE_FORMAT_US,
            maxDate: moment()
        });
    },
    makeWipReconciliationDetailsTable: function (bumpWorkOrderId) {
        var self = this;
        var listSize = 0;

        if (bumpWorkOrderId > 0) {
            var html = '<table class="table table-striped custom-table">';
            html += '<thead>';
            html += '<tr>';
            html += '<th>SL#</th>';
            html += '<th>' + self.options.messages.bumpTapeOutNumber + '</th>';
            ...
            $.get('/admin/getBumpWipInventoryForReconciliation', {bumpWorkOrderId: bumpWorkOrderId}, function (list) {
    ...
    getTableRowHtmlByProductItem: function(item, index, recincileSuccess){
        ...
        html += ' data-product-item-id="'+item.id+'" data-service-execution-plan-id="'+item.serviceExecutionPlanId+'" '
            + ' data-in-progress-qty="'+item.inProgressQuantity+'" data-bumpWorkorder-id="'+item.bumpWorkOrderId+'" '
    ...
    singleReconcile: function(bumpWorkOrderId, productItemId, serviceExecutionId, reconcileQty, rowObject){
        var self = this;
        var validator = self.el.singleReconcileForm.validate();
        Forecast.APP.startLoading();

        $.get('/admin/reconcileBumpService',
            {
                bumpWorkOrderId: bumpWorkOrderId,
    ...
    singleReconcileBtnEvent: function(trObject, reconcileAllBtn){
        var self = this;
        var bumpWorkOrderId = +trObject.attr('data-bumpWorkorder-id');
        ...
        if(isNaN(parseInt(reconcileQty)) || parseInt(reconcileQty) == 0 || !Forecast.APP.isDigits(reconcileQty)){
            ...
            self.singleReconcile(bumpWorkOrderId, productItemId, serviceExecutionPlanId, parseInt(reconcileQty), trObject)
        }
    },
    destroy: function () {
    }
