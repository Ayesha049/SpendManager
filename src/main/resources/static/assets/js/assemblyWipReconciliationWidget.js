$.widget("forecast.assemblyWipReconciliation", {
    options: {
        messages: undefined,
        assemblyWoId: 0
    },
    _create: function () {
        console.log("SMNLOG :: ---------- Assembly WIP Reconciliation widget create ----------");
        var self = this;
        self.el = {};
        self.data = {};
        ...
        self.el.workOrderSelect.change(function () {
            ...
            self.makeWipReconciliationDetailsTable(assemblyWorkOrderId);
        });
        ...
        if(self.options.assemblyWoId > 0){
            console.log("SMNLOG assemblyWoId::"+self.options.assemblyWoId);
            self.el.workOrderSelect.val(self.options.assemblyWoId).trigger('change');
        }
    },
    _init: function () {
        console.log("SMNLOG :: ---------- Assembly WIP Reconciliation init ----------");
        var self = this;

    },
    makeWipReconciliationDetailsTable: function (assemblyWorkOrderId) {
        var self = this;
        var listSize = 0;
        ...
        if (assemblyWorkOrderId > 0) {
            ...
            $.get('/admin/getAssemblyWipInventoryForReconciliation', {assemblyWorkOrderId: assemblyWorkOrderId}, function (list) {
    ...
    getTableRowHtmlByProductItem: function(item, index, recincileSuccess){
        ...
        html += ' data-product-item-id="'+item.id+'" data-service-execution-plan-id="'+item.serviceExecutionPlanId+'" '
            + ' data-in-progress-qty="'+item.inProgressQuantity+'" data-assemblyWorkorder-id="'+item.assemblyWorkOrderId+'" '
    ...
    singleReconcile: function(assemblyWorkOrderId, productItemId, serviceExecutionId, reconcileQty, rowObject){
        var self = this;
        ...
        $.get('/admin/reconcileAssemblyService',
            {
                assemblyWorkOrderId: assemblyWorkOrderId,
    ...
    singleReconcileBtnEvent: function(trObject, reconcileAllBtn){
        var self = this;
        var assemblyWorkOrderId = +trObject.attr('data-assemblyWorkorder-id');
        ...
        if(isNaN(parseInt(reconcileQty)) || parseInt(reconcileQty) == 0 || !Forecast.APP.isDigits(reconcileQty)){
            ...
            self.singleReconcile(assemblyWorkOrderId, productItemId, serviceExecutionPlanId, parseInt(reconcileQty), trObject)
        }
    },
    destroy: function () {
    }
